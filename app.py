...
<style>
    ...
    .subject-block {
        background-color: #fbfbfa; border: 1px solid #ededeb;
        border-radius: 8px; padding: 20px; margin-bottom: 20px;
        box-shadow: 0 1px 2px rgba(0,0,0,0.02);
    }
    .subject-title {
        font-size: 18px; font-weight: 700; color: #37352f;
        margin-bottom: 12px; display: flex; align-items: center; gap: 8px;
    }
    .schedule-box {
        background-color: #f7f7f5; border-radius: 6px;
        padding: 12px 16px; margin-top: 12px; margin-bottom: 16px;
        border-left: 3px solid #60a5fa; display: flex; gap: 24px;
    }
    .schedule-item { font-size: 13px; color: #4b5563; }
    .vertical-timeline {
        position: relative; border-left: 2px solid #e3e2e0;
    ...
    .timeline-dot {
        ...
        background-color: #fff; border: 3px solid #cbd5e1; z-index: 2;
    }
    ...
    .focus-panel {
        background-color: #f7f7f5; border-radius: 12px;
        padding: 32px; text-align: center;
        border: 1px solid #e3e2e0; margin-bottom: 24px;
    }
    .focus-timer { font-size: 48px; font-weight: 700; font-family: monospace; color: #238387; margin: 12px 0; }
    .focus-badge {
        background-color: #e2f3f5; color: #238387;
        padding: 4px 12px; border-radius: 4px;
        font-size: 12px; font-weight: 600; display: inline-block;
    }
    .test-panel {
        background-color: #fff5f5; border: 1px solid #ffe3e3;
    ...
    .consult-container {
        background-color: #fbfbfa; border: 1px solid #e3e2e0;
        border-radius: 8px; padding: 20px; margin-top: 20px; margin-bottom: 16px;
    }
    .consult-user-q {
        font-size: 14px; font-weight: 600; color: #4b5563;
        background-color: #f3f4f6; padding: 10px 14px;
...
SESSION_DEFAULTS = {
    ...
    # [BUG FIX ③] combined_content를 세션에서 관리 — 스코프 오류 방지
    'combined_content': '',
}
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. 파일 텍스트 추출
# [BUG FIX ⑤] UploadedFile 대신 bytes + str을 인자로 받아 캐시 해시 오류 수정
# ──────────────────────────────────────────────────────────────────────────────
...
{kwargs['content'][:4000]}"""
                ...
                st.session_state.focus_detailed_checklist = [
                    l.strip() for l in res.text.split('\n') if l.strip()
                ]

            elif prompt_type == "quiz_questions":
                strict_config = genai.types.GenerationConfig(temperature=0.0)
                p = f"""당신은 대학교 교수이자 시험 출제위원입니다.
아래 [학습 자료]에 적혀있는 문장과 사실만을 근거로 주관식/서술형 퀴즈 3개와 그에 대한 정답을 출제하세요.

작성 수칙 - 매우 중요
1. 학습 자료에 없는 외부 지식, 상식, 일반론은 단 1%라도 절대 개입시키지 마세요.
2. 만약 제공된 학습 자료의 분량이 너무 적어서 3문제를 낼 수 없다면, "학습 자료 내용이 부족하여 문제를 출제할 수 없습니다."라고만 출력하세요.
3. 문제 1, 2는 단답형, 문제 3은 서술형입니다.
4. 반드시 아래 [출제 양식]을 그대로 따르고, '===정답선===' 구분선을 무조건 넣어야 합니다.
5. 별표 같은 마크다운 효과를 전혀 쓰지 말고 순수 텍스트로만 작성하세요.

[출제 양식]
문제 1. (학습 자료에 기반한 문제 내용)

문제 2. (학습 자료에 기반한 문제 내용)

문제 3. (학습 자료에 기반한 문제 내용)
    ...
    if un:
        try:
            ...
            if my_teams:
                ...
                st.info("가입된 스터디 팀이 없습니다. 아래에서 팀을 생성하거나 초대 코드를 입력해 참여하세요.")
        except Exception:
        ...
        with c2:
            ...
            if st.button("공유 워크스페이스 입장"):
                ...
                if res.data:
                    d = res.data[0]
                    ml = d['members']
                    sl = d.get('subjects', {}) or {}
                    if not any(m['name'] == un for m in ml):
                        ml.append({
                            "name": un, "status": "대기",
                            "grade": "-", "days": "-", "total_time": 0
                        })
                        sl[un] = []
                        supabase.table("team").update({
                            "members": ml, "subjects": sl
                        }).eq("invite_code", ci).execute()
                    st.session_state.update({
                        "invite_code": ci,
                        "my_name": un,
                        "page": "dashboard"
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    # [BUG FIX ②] AI 처리 중이거나 focus/test 모드일 때 autorefresh 비활성화
    if (not st.session_state.refresh_lock
            and st.session_state.current_mode == 'dashboard'):
        st_autorefresh(interval=5000, limit=999999, key="global_refresh_engine")

    res = supabase.table("team").select("*").eq(
        "invite_code", st.session_state.invite_code
    ).execute()

    # [BUG FIX ⑧] DB 연결 실패 시 사용자에게 안내
    if not res.data:
        st.error("팀 데이터를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.")
        if st.button("로그아웃"):
            st.session_state.update({"invite_code": "", "page": "gate"})
            st.rerun()
        st.stop()

    data = res.data[0]

    # [BUG FIX ①] subjects가 list로 내려온 경우 복구 — DB 저장 후 rerun
    if isinstance(data.get('subjects'), list):
        recovered = {st.session_state.my_name: data['subjects']}
        supabase.table("team").update({
            "subjects": recovered
        }).eq("invite_code", st.session_state.invite_code).execute()
        data['subjects'] = recovered
        st.rerun()  # 클린 상태로 재진입

    # subjects가 None인 경우 방어
    if data.get('subjects') is None:
        data['subjects'] = {}

    # ── 사이드바 ──────────────────────────────────────────────────────────────
    st.sidebar.markdown(
        f"<div class='notion-header' style='font-size:20px;'>📂 {data['team_name']}</div>",
        unsafe_allow_html=True
    )
    st.sidebar.markdown(
        f"<div class='notion-sub'>사용자: {st.session_state.my_name} 님</div>",
        unsafe_allow_html=True
    )
    ...
    if st.session_state.current_mode in ['dashboard', 'result']:
        menu = st.sidebar.radio(
            "내비게이션",
            ["내 학습 보드 (메인)", "👥 팀원 실시간 페이스", "공유 게시판", "AI 진로 및 학업 상담"]
        )
    else:
    ...
    if st.sidebar.button("🚪 워크스페이스 로그아웃"):
        st.session_state.update({
            "invite_code": "", "page": "gate", "current_mode": "dashboard",
            "current_ai_plan": "", "current_ai_quiz": "", "current_ai_quiz_answers": "",
            "current_ai_consult_q": "", "current_ai_consult_a": "",
            "timer_running": False, "refresh_lock": False,
        })
        st.rerun()
    ...
    if st.session_state.current_mode == 'dashboard':
        ...
        if menu == "내 학습 보드 (메인)":
            ...
            st.markdown("<div class='notion-sub'>각 과목 카드를 클릭하여 펼치면, 수직선 마일스톤 점과 동기화된 일차별 AI 계획을 확인할 수 있습니다.</div>", unsafe_allow_html=True)

            my_subs = data['subjects'].get(st.session_state.my_name, []) or []
            ...
            if my_subs:
                ...
                for sub in my_subs:
                    ...
                    with st.container():
                        ...
                        <div class='subject-block'>
                            <div class='subject-title'>📚 {sub_name}
                                <span style='font-size:13px; color:#238387; font-weight:normal;'>
                                    — 이수율 {progress_percent}% (Day {current_day}/{total_days}일차)
                                </span>
                            </div>
                        ...
                        with st.expander(f"🔍 {sub_name} 전체 일차별 로드맵", expanded=True):
                            st.markdown("<div class='vertical-timeline'>", unsafe_allow_html=True)
                            for d_i in range(1, total_days + 1):
                                mission_desc = sub_missions.get(
                                    d_i,
                                    f"{sub_name} {d_i}일차 교안 핵심 텍스트 분석 및 모의평가 수행"
                                )
                                if d_i < current_day:
                                    dot_class, badge_class, status_label = "dot-done", "nb-done", "✅ 완수"
                                elif d_i == current_day:
                                    dot_class, badge_class, status_label = "dot-active", "nb-active", "🔥 진행 중"
                                else:
                                    dot_class, badge_class, status_label = "", "nb-waiting", "🔒 대기"
                ...
                st.info("현재 등록된 학습 과목이 없습니다. 아래에서 과목을 추가하고 AI 맞춤 플랜을 생성해 보세요!")
            ...
            with c_right:
                if my_subs:
                    ...
                    st.session_state.input_manual_text = st.text_area(
                        "학습 교안 본문 및 AI 세부 지시문 입력",
                        value=st.session_state.input_manual_text,
                        height=100,
                        placeholder="요약할 텍스트를 붙여넣거나 세부 지시 사항을 입력하세요."
                    )

                    up_files = st.file_uploader(
                        f"{target_sub} 자료 업로드 (여러 개 선택 가능)",
                        type=['pdf', 'txt'],
                        key="uploader_dash",
                        accept_multiple_files=True
                    )

                    # [BUG FIX ⑤] bytes + type으로 캐시 호출
                    extracted = "".join(
                        [extract_text(f.getvalue(), f.type) for f in up_files]
                    ) if up_files else ""

                    # [BUG FIX ③] combined_content를 세션 상태에 저장
                    st.session_state.combined_content = (
                        f"{st.session_state.input_manual_text}\n{extracted}".strip()
                    )

                    cd1, cd2 = st.columns(2)
                    days = cd1.number_input(
                        "목표 학습 기간 (일)", 1, 100,
                        value=st.session_state.input_days
                    )
                    st.session_state.input_days = days
                    grade = cd2.selectbox(
                        "달성 목표 학점 레벨",
                        ["A+", "B+", "Pass"],
                        index=["A+", "B+", "Pass"].index(st.session_state.input_grade)
                    )
                    ...
                    if st.button("🤖 AI 맞춤형 일차별 계획 설계", type="primary", use_container_width=True):
                        if st.session_state.combined_content:
                            st.session_state[f"saved_doc_{target_sub}"] = st.session_state.combined_content
                            ...
                            run_ai_engine(
                                ...
                                grade=grade,
                                days=days,
                                content=st.session_state.combined_content
                            )
                            gc.collect()
                        else:
            ...
            if my_subs:
                st.divider()
                st.markdown("### 🚀 오늘자 미션 집중 런처")

                cl1, cl2, cl3 = st.columns([1.5, 1, 1])
                with cl1:
                    selected_sub_to_study = st.selectbox(
                        "오늘 몰입할 과목 고르기",
                        [s['name'] for s in my_subs],
                        key="study_selector"
                    )
                ...
                with cl2:
                    chosen_day = st.selectbox(
                        "진행할 목표 일차",
                        [i for i in range(1, max_sub_day + 1)],
                        index=min(current_sub_day - 1, max_sub_day - 1)
                    )

                with cl3:
                    st.write("")
                    st.write("")
                    if st.button("🔥 몰입 모드 가동", type="primary", use_container_width=True):
                        ...
                        # [BUG FIX ③] combined_content 스코프 오류 수정 — 세션에서 안전하게 참조
                        saved_doc = st.session_state.get(f"saved_doc_{selected_sub_to_study}", "")
                        st.session_state.saved_study_content = (
                            saved_doc if saved_doc
                            else st.session_state.combined_content if st.session_state.combined_content
                            else "기본 학업 개념"
                        )
            ...
            for m_block in data['members']:
                ...
                st.markdown(f"""
                <div class='subject-block' style='border-left: 4px solid #238387;'>
                    ...
                    <div style='margin-top:8px; font-size:13px; color:#7c7b77;'>
                        🎯 목표 레벨: {m_block.get('grade', '-')} | 설정 기간: {m_block.get('days', '-')} |
                        ⏱️ 오늘 누적 집중 시간: {m_block.get('total_time', 0)}분
                    </div>
                </div>
                """, unsafe_allow_html=True)
                sub_list = data['subjects'].get(m_block['name'], []) or []
                if sub_list:
            ...
            with st.form("b_form_notion"):
                t = st.text_input("게시글 제목")
                c = st.text_area("내용 공유")
                if st.form_submit_button("보드에 포스팅 등록"):
                    if t and c:
                        ps = data['posts']
                        ps.append({"title": t, "content": c, "author": st.session_state.my_name})
                        supabase.table("team").update({
                            "posts": ps
                        }).eq("invite_code", st.session_state.invite_code).execute()
                        st.rerun()
            ...
            user_query = st.text_area(
                "현재 학업 설계나 진로 선택, 슬럼프 고민에 대해 자유롭게 입력해 주세요.",
                height=150,
                placeholder="예: 전공 공부가 적성에 안 맞는 것 같아요. / 학점 관리 요령을 알고 싶어요."
            )
            if st.button("멘토 AI에게 고민 솔루션 신청", type="primary", use_container_width=True):
                if user_query:
            ...
            if st.session_state.current_ai_consult_a:
                ...
                <div class='consult-container'>
                    <div style='font-size: 16px; font-weight: 700; color: #37352f; margin-bottom: 12px;'>🔮 AI 멘토의 1:1 맞춤 솔루션</div>
                    <div class='consult-user-q'>👤 제출한 고민 내역:<br>{st.session_state.current_ai_consult_q}</div>
                    <div class='consult-ai-a'>🤖 AI 조언:<br><br>{st.session_state.current_ai_consult_a.replace(chr(10), '<br>')}</div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.info("💡 위 입력창에 고민을 작성하고 신청 버튼을 누르시면, AI 멘토의 1:1 맞춤 피드백이 여기에 출력됩니다.")

    # =========================================================================
    # MODE 2: 몰입 모드
    # =========================================================================
    elif st.session_state.current_mode == 'focus':
        st.markdown("<div class='notion-header'>⚡ IMMERSION FOCUS MODE</div>", unsafe_allow_html=True)

        # [BUG FIX ⑦] 타이머: start_time이 None이면 방어
        ...
        <div class='focus-panel'>
            ...
            <div style='color: #7c7b77; font-size:13px;'>모든 외부 알림을 차단하고 집중 상태를 유지하세요.</div>
        </div>
        """, unsafe_allow_html=True)

        # focus 모드에서만 빠른 autorefresh (타이머 표시용)
        st_autorefresh(interval=1000, limit=999999, key="focus_timer_refresh")
        ...
        if st.session_state.current_ai_plan:
            ...
            if not target_day_text.strip():
                for line in lines:
                    if (f"Day {st.session_state.active_day}" in line or
                            f"Day{st.session_state.active_day}" in line) and ":" in line:
                        target_day_text += line.split(":", 1)[1].strip() + "\n"

        if not target_day_text.strip():
            target_day_text = f"Day {st.session_state.active_day}: 교안 핵심 개념 정독 및 노트 정리"
        ...
        if not st.session_state.focus_detailed_checklist:
            if st.button("AI 튜터에게 미션 쪼개기 요청 (상세 체크리스트 생성)", use_container_width=True):
                study_data = st.session_state.saved_study_content or "기본 학업 개념"
            ...
            for idx, m_line in enumerate(st.session_state.focus_detailed_checklist):
                ...
                if clean_line.strip():
                    st.checkbox(
                        f"{clean_line.strip()}",
                        key=f"mission_chk_{st.session_state.active_subject}_{st.session_state.active_day}_{idx}"
                    )
        ...
        for msg in st.session_state.focus_chat_history:
            if msg["role"] == "user":
                st.info(f"👤 나: {msg['content']}")
            else:
                st.success(f"🤖 튜터 AI: {msg['content']}")

        with st.form("focus_chat_form", clear_on_submit=True):
            c_col1, c_col2 = st.columns([4, 1])
            with c_col1:
                user_q = st.text_input(
                    "질문 입력",
                    label_visibility="collapsed",
                    placeholder="자료에 대해 궁금한 점을 입력하세요"
                )
            with c_col2:
                submitted = st.form_submit_button("질문 전송", use_container_width=True)
    ...
    # =========================================================================
        ...
        st_autorefresh(interval=1000, limit=999999, key="test_timer_refresh")
        ...
        with test_col_l:
            ...
            if st.session_state.current_ai_quiz:
                ...
                st.info("AI가 학습 자료 분석을 마치고 시험지를 전송 중입니다. 잠시만 기다려 주세요... 📝")

        with test_col_r:
            st.markdown("<div class='omr-container'><div class='omr-title'>📟 DIGITAL OMR CARD</div></div>", unsafe_allow_html=True)
            ...
            if st.button("📥 [최종 답안 전송] 시험 종료", type="primary", use_container_width=True):
                my_subs_update    = data['subjects'].get(st.session_state.my_name, []) or []
                ...
                # [BUG FIX ⑥] 전체 subjects dict 업데이트
                all_s = data['subjects']
                all_s[st.session_state.my_name] = my_subs_update
                supabase.table("team").update({
                    "subjects": all_s, "members": ml
                }).eq("invite_code", st.session_state.invite_code).execute()

                st.session_state.current_mode = 'result' if is_course_finished else 'result_daily'
                st.rerun()
    ...
    # =========================================================================
        ...
        tab_ans, tab_solution = st.tabs(["📥 내가 마킹한 OMR 제출본 확인", "🔍 AI 출제 정답 및 해설지"])
        with tab_ans:
        ...
        if st.button("🔄 검증 완료 및 대시보드로 돌아가기", type="primary", use_container_width=True):
            ...
            # [BUG FIX ④] st.user_answers → st.session_state.user_answers
            st.session_state.user_answers         = {}
            st.rerun()
    ...
    # =========================================================================
        ...
        st.markdown(
            f"<div class='notion-sub'>축하합니다! {st.session_state.active_subject} 과목의 모든 일차 학습 과정과 최종 평가가 공식 종료되었습니다.</div>",
            unsafe_allow_html=True
        )
        st.markdown(f"""
        <div class='report-card'>
            <div style='font-size: 14px; font-weight: 600; color: #64748b; margin-bottom: 8px;'>학사 이수 인증 성적표</div>
            <div style='font-size: 24px; font-weight: 700; color: #334155; margin-bottom: 4px;'>과목명: {st.session_state.active_subject}</div>
            <div style='font-size: 14px; color: #94a3b8; margin-bottom: 16px;'>전 과정 최종 완료</div>
        ...
        st.write("")
        tab_final_ans, tab_final_sol = st.tabs(["📥 최종 졸업 고사 제출 답안 확인", "🔍 출제 정답 및 해설지"])
        with tab_final_ans:
