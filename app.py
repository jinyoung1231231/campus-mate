...
{kwargs['content'][:4000]}"""
                ...
                st.session_state.current_ai_quiz = "" # 새 계획 생성 시 이전 퀴즈 초기화
...
if st.session_state.page == 'gate':
    ...
    if un:
        try:
            ...
            if my_teams:
                st.write("내 팀 목록")
                for t in my_teams:
                    if st.button(f"집 입장: {t['team_name']}", key=f"t_{t['invite_code']}"):
    ...
    # 실시간 시간 새로고침 및 타이머 갱신용 오토 리프레시 (2초 간격)
    ...
    if data:
        ...
        with col_l:
            if menu == "내 학습 & AI":
                ...
                if st.session_state.timer_running:
                    ...
                    st.metric(label="⌛ 현재 누적 몰입 시간", value=f"{h:02d}:{m:02d}:{s:02d}")
                else:
                    h = st.session_state.elapsed_time // 3600
                    m = (st.session_state.elapsed_time % 3600) // 60
                    s = st.session_state.elapsed_time % 60
                    st.metric(label="⏱️ 대기 중인 타이머", value=f"{h:02d}:{m:02d}:{s:02d}")
                ...
                if my_subs:
                    ...
                    manual_text = st.text_area("자료 직접 입력 및 AI 지시사항 작성", height=100)
                    ...
                    days = c_d.number_input("학습 기간 설정 (일)", 1, 100, 7)
                    grade = c_g.selectbox("목표 성적 선택", ["A+", "B+", "Pass"])
                    ...
                    with c1:
                        ...
                        if st.button("공부 시작", use_container_width=True, disabled=st.session_state.timer_running):
                            ...
                            st.rerun()
                    with c2:
                        # 공부 종료 액션 (시간 누적 및 해당 과목 맞춤형 퀴즈 출제)
        ...
        # 우측 결과단 영역 (상시 보존 및 하이라이트 시각화)
        with col_r:
            if menu == "내 학습 & AI" or menu == "팀원 상세 과목" or menu == "게시판":
                st.header("🤖 AI 학습 일정 검증 센터")
                st.divider()
                
                if st.session_state.current_ai_plan:
                    st.info("💡 설정한 학습 진도(Day)에 따라 현재 소화해야 할 계획 카드가 아래에 강조 표시됩니다.")
                    
                    # 텍스트로 보존된 일정을 라인별로 읽어 직관적인 블록 카드로 파싱
                    lines = st.session_state.current_ai_plan.split('\n')
                    for line in lines:
                        if not line.strip(): continue
                        
                        # 사용자가 좌측 콘솔에서 선택한 Day 번호와 매칭되는지 확인
                        if menu == "내 학습 & AI" and selected_day in line:
                            # 현재 공부하고 있는 일차는 주황색 불꽃 하이라이트 카드로 노출
                            st.warning(f"🔥 현재 진행 중인 목표 분량\n\n{line}")
                        else:
                            # 나머지 일차는 일반 텍스트 박스로 깔끔하게 나열
                            st.code(line, language="text")
                else:
                    st.info("좌측 콘솔에서 자료 입력 후 AI 맞춤 일정을 생성하면 여기에 영구 보존됩니다.")
                
                # 공부가 종료된 후에만 하단에 복습 퀴즈가 누적 표시됨 (일정을 덮어쓰지 않음)
