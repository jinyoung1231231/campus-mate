...
def run_ai(prompt_type, **kwargs):
    with st.spinner("AI가 학습 일정을 작성 중입니다... 📝"):
        try:
...
if st.session_state.page == 'gate':
    ...
    if data:
        ...
        with col_l:
            if menu == "📚 내 학습 & AI":
                ...
                if my_subs:
                    ...
                    if st.button("🪄 AI 맞춤 일정 생성", type="primary", use_container_width=True):
                ...
                for m in data['members']:
                    with st.expander(f"{'🔥' if '중' in m['status'] else '✅'} {m['name']} 님"):
                        ...
                        st.write(f"목표: {m.get('grade', '-')} | 남은일: {m.get('days', '-')}일")
                        st.write("**📚 등록된 모든 과목**")
        ...
        with col_r:
            ...
            else: st.info("AI의 텍스트 답변이 여기에 표시됩니다.")
