...
def init_connection():
    try:
        ...
        # [핵심] 연결 가능한 모델 명칭을 순차적으로 테스트
        selected_model = None
        # 서버 환경에 따라 인식하는 이름이 다르므로 4가지를 모두 시도
        candidates = ['gemini-1.5-flash', 'models/gemini-1.5-flash', 'gemini-pro', 'models/gemini-pro']
...
def run_ai(prompt_type, **kwargs):
    if not model:
        st.error("🤖 AI 모델이 아직 연결되지 않았습니다. API 키 권한을 확인해 주세요.")
        return
    
    with st.spinner("AI 분석 중..."):
        try:
            if prompt_type == "plan":
                p = f"목표성적:{kwargs['grade']}, 기간:{kwargs['days']}일. 다음 자료를 분석해서 상세 일정을 짜줘: {st.session_state.file_content[:4000]}"
...
if st.session_state.page == 'gate':
    ...
    if data:
        ...
        menu = st.sidebar.radio("메뉴", ["📚 내 학습 & AI", "👥 팀원별 상세 과목", "📋 게시판", "💡 진로상담"])
