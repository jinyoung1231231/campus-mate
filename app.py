        ...
        with col_l:
            if menu == "내 학습 & AI":
                ...
                if st.session_state.timer_running:
                    ...
                    st.markdown(f"""<div class='timer-container'><div>⏳ 현재 실시간 몰입 시간</div><div style='font-size: 32px; margin-top: 5px;'>{h:02d}:{m:02d}:{s:02d}</div></div>""", unsafe_allow_html=True)
                else:
                    h = st.session_state.elapsed_time // 3600; m = (st.session_state.elapsed_time % 3600) // 60; s = st.session_state.elapsed_time % 60
                    st.markdown(f"""<div class='timer-container' style='background: linear-gradient(135deg, #475569, #64748b);'><div>⏱️ 대기 중인 타이머</div><div style='font-size: 32px; margin-top: 5px;'>{h:02d}:{m:02d}:{s:02d}</div></div>""", unsafe_allow_html=True)
