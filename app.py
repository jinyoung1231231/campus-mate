...
def init_connection():
    try:
        ...
        # 모델 직접 지정 (가장 범용적인 명칭 사용)
        # 만약 여기서 에러가 난다면 API 키 자체가 유효하지 않을 확률이 높습니다.
...
def run_ai(prompt_type, **kwargs):
    """AI를 호출하고 결과를 즉시 화면에 반영하는 핵심 함수"""
    if not model:
        st.error("🤖 AI 모델이 초기화되지 않았습니다. Secrets에 GEMINI_API_KEY가 있는지 확인하세요.")
        return
    
    with st.spinner("AI가 분석 중입니다..."):
        try:
            if prompt_type == "plan":
                p = f"목표:{kwargs['grade']}, 남은일수:{kwargs['days']}일. 아래 자료를 토대로 학습 일정을 짜줘: {st.session_state.file_content[:4000]}"
            elif prompt_type == "quiz":
                p = f"아래 공부 자료에서 핵심 퀴즈 3개와 정답을 내줘: {st.session_state.file_content[:4000]}"
            elif prompt_type == "consult":
                p = f"상담 답변: {kwargs['q']}"
            
            # AI 호출 (이 지점에서 에러 발생 시 API 키 문제일 가능성 높음)
...
if st.session_state.page == 'gate':
    ...
    if not st.session_state.my_name: # 세션 끊김 방지
    ...
    if data:
        # [기능 1] 초대 코드 및 팀 정보 사이드바 상시 노출
        st.sidebar.success(f"🎫 초대 코드: **{data['invite_code']}**")
        st.sidebar.title(f"🏫 {data['team_name']}")
        
        menu = st.sidebar.radio("메뉴", ["📚 내 학습 & AI", "👥 팀원별 과목 정보", "📋 게시판", "💡 진로상담"])
        if st.sidebar.button("⬅️ 로그아웃"): st.session_state.page = 'gate'; st.rerun()

        col_main, col_ai = st.columns([1, 1])

        with col_main:
            if menu == "📚 내 학습 & AI":
                st.header("📚 내 공부 & AI 플래너")
                ...
                if my_subs:
                    sel_sub = st.selectbox("현재 공부 중인 과목", [s['name'] for s in my_subs])
                    up_file = st.file_uploader("자료 업로드(PDF/TXT)", type=['pdf', 'txt'])
                    if up_file:
                        st.session_state.file_content = extract_text(up_file)
                        st.success("✅ 파일 내용 인식 완료!")

                    st.divider()
                    st.markdown("#### 🎯 목표 성적 및 기간 세팅")
                    ...
                    if st.button("🪄 AI 맞춤 일정 생성", use_container_width=True):
                        ...
                        else: st.warning("분석할 파일을 먼저 업로드해주세요.")
            ...
            # [기능 2] 팀원별 과목 정보 열람 (상세화)
            elif menu == "👥 팀원별 과목 정보":
                st.header("👥 팀원 학습 상세 현황")
                st.info("팀원의 이름을 누르면 등록된 모든 과목을 확인할 수 있습니다.")
                for m in data['members']:
                    with st.expander(f"{'🔥' if '중' in m['status'] else '✅'} {m['name']} 님의 정보"):
                        st.write(f"현재 상태: **{m['status']}**")
                        st.write(f"🎯 목표: **{m.get('grade', '-')}** | ⏳ 남은 기간: **{m.get('days', '-')}**")
                        st.divider()
                        st.write("**📚 등록된 과목 리스트**")
        ...
        with col_ai:
            ...
            else: st.info("AI의 학습 플랜, 상담, 퀴즈 결과가 여기에 표시됩니다.")
