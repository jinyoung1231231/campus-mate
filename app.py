...
# 7. 핵심 메인 워크스페이스 렌더링 
    ...
    if data:
        ...
        # =========================================================================
        # MODE 3: 시험 모드 (완전 격리 및 진입 시 자동 생성 트리거 구축 완료)
        # =========================================================================
            ...
            # 문제를 수동으로 기다릴 필요 없이 데이터가 없다면 첫 진입 시 자동 런칭
            if not st.session_state.current_ai_quiz:
                ...
                st.warning("⚠️ 시험지를 분석하여 생성하고 있습니다. 잠시만 기다려 주세요...")
                ...
                # 아직 lock이 걸려있지 않은 최초 진입 상태라면 자동으로 퀴즈 출제 함수 즉시 기동
