...
def run_ai(prompt_type, **kwargs):
    ...
    with st.spinner("AI가 분석 중입니다..."):
        try:
            if prompt_type == "plan":
                p = f"목표성적:{kwargs['grade']}, 기간:{kwargs['days']}일. 아래 자료 분석 스케줄 생성: {st.session_state.file_content[:3500]}"
            elif prompt_type == "quiz":
                p = f"아래 자료에서 핵심 퀴즈 3개와 정답 생성: {st.session_state.file_content[:3500]}"
...
if st.session_state.page == 'gate':
    st.title("🔥 Check-Mate")
    st.markdown("#### 닉네임을 입력해 내 팀들을 확인하거나 새로 만드세요.")
    ...
    if un:
        ...
        if my_teams:
            st.subheader("📋 내 팀 목록")
            for t in my_teams:
                if st.button(f"🏠 {t['team_name']} 입장", key=f"btn_{t['invite_code']}"):
    ...
    if data:
        ...
        menu = st.sidebar.radio("메뉴", ["📚 내 학습 & AI", "👥 팀원 과목 정보", "📋 게시판", "💡 상담"])
