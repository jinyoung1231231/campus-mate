...
def init_connection():
    ...
    # [에러 해결 핵심] 404를 피하기 위해 가장 호환성 높은 이름으로 시도
    m = None
    # 1순위: 1.5-flash, 2순위: 1.0-pro (안정성)
    ...
    if m is None:
        # 모든 시도가 실패할 경우 사용자에게 알림
        st.error("Gemini 모델에 연결할 수 없습니다. API 키를 다시 확인하거나 구글 AI 스튜디오에서 키가 활성 상태인지 확인해주세요.")
