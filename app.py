...
def init_connection():
    try:
        ...
        # [해결] 404 에러 방지: 작동하는 모델 이름을 찾을 때까지 시도
        # 서버 환경마다 인식하는 이름이 다르므로 후보군을 모두 찔러봅니다.
        m_candidates = ['gemini-1.5-flash', 'models/gemini-1.5-flash', 'gemini-pro']
        selected_model = None
        
        for name in m_candidates:
            try:
                m = genai.GenerativeModel(name)
                # 1토큰이라도 생성되는지 "진짜" 연결 확인
...
def run_ai(prompt_type, **kwargs):
    ...
    with st.spinner("AI가 분석 중입니다..."):
        try:
            if prompt_type == "plan":
                p = f"목표:{kwargs['grade']}, 기간:{kwargs['days']}일. 자료 요약 및 일정 생성: {st.session_state.file_content[:3500]}"
            elif prompt_type == "quiz":
                p = f"자료 기반 퀴즈 3개와 정답: {st.session_state.file_content[:3500]}"
...
if st.session_state.page == 'gate':
    st.title("🔥 Check-Mate")
    st.markdown("#### 닉네임을 입력해 내가 속한 팀들을 확인하세요.")
    
    un = st.text_input("사용자 닉네임 입력 (로그인)")
    
    if un:
        # DB에서 해당 닉네임이 포함된 모든 팀 찾기
        try:
            all_data = supabase.table("team").select("*").execute().data
            my_teams = [t for t in all_data if any(m['name'] == un for m in t['members'])]
            
            if my_teams:
                st.subheader("📋 참여 중인 팀 목록")
                for t in my_teams:
                    if st.button(f"🏠 {t['team_name']} 입장", key=f"team_{t['invite_code']}"):
                        st.session_state.update({"invite_code": t['invite_code'], "my_name": un, "page": "dashboard"})
                        st.rerun()
            else:
                st.info("아직 소속된 팀이 없습니다. 아래에서 팀을 생성하거나 코드를 입력하세요.")
        except:
...
# [대시보드: 메인 기능]
    ...
    if data:
        ...
        menu = st.sidebar.radio("메뉴", ["📚 학습 & AI", "👥 팀원 상세현황", "📋 게시판", "💡 상담"])
        
        if st.sidebar.button("⬅️ 다른 팀으로 이동 (나가기)"):
